...
clicker_themes = [
    ("Burger Clicker", "burger_clicker", "🍔", "burgers", "#f59e0b", "#d97706"),
    ...
    ("Candy Factory", "candy_factory", "🍭", "candies", "#f472b6", "#be185d"),
...
<head>
    <meta charset="UTF-8">
    <title>{title} – The Pizza Edition</title>
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box }}
        ...
        h1 {{ color: {color1}; font-size: 2rem; text-shadow: 0 0 20px {color1}; margin-bottom: 4px }}
        ...
        #click-btn {{ width: 150px; height: 150px; border-radius: 50%; background: radial-gradient(circle at 35% 35%, {color1}, {color2}); border: 4px solid {color1}; cursor: pointer; font-size: 78px; display: flex; align-items: center; justify-content: center; box-shadow: 0 0 30px rgba(0,0,0, .5); user-select: none; transition: transform .1s }}
        #click-btn:active {{ transform: scale(.92) }}
        .xp-bar {{ height: 7px; background: rgba(255, 255, 255, .15); border-radius: 4px; width: 150px }}
        .xp-fill {{ height: 100%; background: linear-gradient(90deg, {color1}, {color2}); border-radius: 4px; transition: width .2s }}
        #lvl-lbl {{ font-size: .78rem; color: {color1}; text-align: center }}
        #shop {{ flex: 1; min-width: 260px; background: rgba(255, 255, 255, .04); border: 1px solid {color2}; border-radius: 20px; padding: 20px; max-height: 70vh; overflow-y: auto }}
        #shop h2 {{ color: {color1}; font-size: 1.1rem; margin-bottom: 14px }}
        ...
        .uo {{ font-size: .72rem; color: {color1} }}
        #toast {{ position: fixed; top: 20px; right: 20px; background: rgba(0,0,0, .9); border:1px solid {color1}; color: #fff; padding: 10px 18px; border-radius: 10px; font-weight: 700; opacity: 0; transition: opacity .3s; pointer-events: none; z-index: 99 }}
    </style>
</head>
<body>
    <h1>{emoji} {title}</h1>
    <div id="counter">0 {resource}</div>
    <div id="cps">0/sec | Level 1</div>
    <div class="layout">
        <div id="click-zone">
            <div id="click-btn" onclick="clickItem()">{emoji}</div>
    ...
    <script>
        ...
        function loop(ts) {{
            ...
            last = ts;
            document.getElementById('counter').textContent = fmt(items) + ' {resource}';
...
for title, filename, emoji, resource, c1, c2 in clicker_themes:
    # 1. Create file
    filepath = os.path.join("games/clicker", f"{filename}.html")
    ...
    # 2. Add to HTML insert
    html_inserts.append(f"""                    <div class="game-card" onclick="window.open('games/clicker/{filename}.html','_blank')">
